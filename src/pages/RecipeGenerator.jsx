...
function RecipeGenerator({ userPreferences, onRecipeReady }) {
  ...
  // All parsing logic remains the same
  const parseIngredients = (recipeText) => {
    if (!recipeText) return [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setIsLoading(false); // New: Set loading to false regardless of success or failure
    }
  ...
  return (
    ...
    >
      {/* Buttons */}
      <div className="flex flex-col items-center gap-4 mt-4">
        {/* New: Buttons use the orange color palette */}
        <button
          className="btn btn-primary bg-orange-500 hover:bg-orange-600 text-white transition-colors duration-300"
          onClick={() => {
            setUsePreferencesMode(false);
            setShowForm(true);
          }}
        ...
        <button
          className="btn btn-secondary bg-gray-200 hover:bg-gray-300 text-gray-800 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-100 transition-colors duration-300"
          onClick={handleUsePreferencesClick}
        >
          Use My Preferences
        </button>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-2xl w-full max-w-md relative transition-colors duration-300">
            <button
              className="absolute top-4 right-4 text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-600 transition-colors duration-300"
              onClick={() => {
                setShowForm(false);
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              {/* New: Submit button uses orange color */}
              <button type="submit" className="btn btn-primary w-full bg-orange-500 hover:bg-orange-600 text-white">
                Generate Recipe
              </button>
            </form>
          </div>
      ...
      {/* Recipe + Shopping List */}
      <div className="w-full mt-20 gap-10 grid grid-cols-1 lg:grid-cols-3">
        {/* Conditional Rendering: Show spinner if loading, otherwise show content */}
        {isLoading ? (
          <div className="lg:col-span-3 flex items-center justify-center h-64">
            <ClipLoader color="#F97316" loading={isLoading} size={50} />
          </div>
        ) : (
          <>
            {/* Recipe Output */}
